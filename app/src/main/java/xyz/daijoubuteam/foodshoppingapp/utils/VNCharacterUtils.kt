...
object VNCharacterUtils {
    ...
    fun removeAccent(ch: Char): Char {
        var ch = ch
        val index: Int = Arrays.binarySearch(VNCharacterUtils.SOURCE_CHARACTERS, ch)
        if (index >= 0) {
            ch = VNCharacterUtils.DESTINATION_CHARACTERS.get(index)
        }
        return ch
    }
    ...
    fun removeAccent(str: String?): String {
        ...
        for (i in 0 until sb.length) {
            sb.setCharAt(i, VNCharacterUtils.removeAccent(sb[i]))
        }
        return sb.toString()
    }
}
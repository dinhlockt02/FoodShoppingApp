...
class SearchRepository {
    ...
    suspend fun getEateryById(id: String): Eatery? {
        val eatery =
            db.collection("eateries").document(id).get().await().toObject(Eatery::class.java)
        val products = db.collection("eateries").document(id).collection("products").get().await()
    ...
    suspend fun searchEateries(searchText: String?): List<Eatery> {
        ...
        val eateries =
            db.collection("eateries").get().await().toObjects(Eatery::class.java).map { eatery ->
                if (eatery.id != null) {
                    val products =
                        db.collection("eateries").document(eatery.id).collection("products").get()
                            .await().toObjects(Product::class.java).filter { product ->
                            VNCharacterUtils.removeAccent(product.name)
                                .contains(removeAccentSearchText, ignoreCase = true)
                        }.take(2)
                ...
                return@filter VNCharacterUtils.removeAccent(eatery.name)
                    .contains(removeAccentSearchText, ignoreCase = true)
            }.toList()
        return eateries;
    }
